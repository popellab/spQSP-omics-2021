...
#include "TNBC_omics/SP_QSP_TNBC/core/Param.h"
#include "TNBC_omics/SP_QSP_TNBC/ode/Param.h"
#include "TNBC_core.h"
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		desc.add_options()
			...
			("antigen-file,a", po::value<std::string>(&_inputAntigen), "antigen file name")
	...
	TNBC_Core simulation(_antigenFile);
	...
	for (size_t slice = sliceStart; slice <= _nrStep; slice++)
	{
		...
		if (_saveStats && slice % _saveStatsInterval == 0 && slice != _nrStep)
		{
			...
			simulation.write_TumorVolume(slice);
		}
