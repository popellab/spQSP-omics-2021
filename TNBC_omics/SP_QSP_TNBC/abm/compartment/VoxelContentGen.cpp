...
void VoxelContentGen::setup( double pstem, int xlim, int ylim, int zlim,
	...
	// We arbitrarily chose to use a uniform distribution over the div counts between 1 and dmax
	for (size_t i = 1; i <= dmax; i++)
	{
		_celltype_cdf[i] = pstem + (1.0 - pstem)*i/(dmax);
	}
	_celltype_cdf[dmax + 1] = 1.0;
	_celltype_cdf[dmax+2] = 1.0;
	/*
	std::cout << "cancer cell cdf:" << std::endl;
	for (size_t i = 0; i < _celltype_cdf.size(); i++)
