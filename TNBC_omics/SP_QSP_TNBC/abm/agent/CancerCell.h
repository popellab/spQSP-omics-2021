...
public:
	...
	// ! set antigens to cancer cell
	void setAntigenVec(std::vector<bool> agVec) { _antigen_vec = agVec;  };
	// ! get antigens from cancer cell
	std::vector<bool> const& getAntigenVec() const { return _antigen_vec; };
...
private:
	...
	//! vector contains antigen that cancer cells have
	std::vector<bool> _antigen_vec;
...
inline void CancerCell::serialize(Archive & ar, const unsigned int /* version */){
	...
	ar & BOOST_SERIALIZATION_NVP(_antigen_vec);
}
