...
public:
	...
	//! get TCR clone
	int const& getTCR() { return _TCR_clone; } ;
	...
	//! it is returning a boolean to determine whether the t-cell is successfully recruited (True: Sufficient; False: insufficient Tcell)
	void set_TCR_clone(const std::vector<double> Tcell_Distribution, const double total_concentration);

	//! it is returning a boolean to determine whether the t-cell is successfully recruited (True: Sufficient; False: insufficient Tcell)
	//bool set_TCR_clone(const std::vector<double> Tcell_Distribution, const double total_concentration, const double abm_scaler, const std::vector<double> var_abm_to_qsp_teff, double num_rec);
...
private:
	...
	// ! The clonality of T cell receptor, matching the antigen vector in Cancer cell
	int _TCR_clone;
...
inline void TCell::serialize(Archive & ar, const unsigned int /* version */) {
	...
	ar & BOOST_SERIALIZATION_NVP(_TCR_clone);
}
