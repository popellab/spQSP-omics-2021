...
const char* _description[][PARAM_DESCRIPTION_FIELD_COUNT] =
{
	...
	{ "Param.ABM.Treg.lifespanSD", "days", "pos" },
...
void Param::processInternalParams(){
	...
	_paramFloatInternal[PARAM_TREG_LIFE_MEAN_SLICE] = _paramFloat[PARAM_TREG_LIFE_MEAN]
		/ _paramFloat[PARAM_SEC_PER_TIME_SLICE] * SEC_PER_DAY;

	_paramFloatInternal[PARAM_TREG_LIFE_SD_SLICE] = _paramFloat[PARAM_TREG_LIFE_SD]
		/ _paramFloat[PARAM_SEC_PER_TIME_SLICE] * SEC_PER_DAY;
...
void Param::update_from_qsp(void){
	...
	// cell
	_paramFloatInternal[PARAM_CELL] = QP(9);	
	...
	_paramFloatInternal[PARAM_TREGMAX] = QP(594);		

	// maximum concentration of MDSC per volume
	_paramFloatInternal[PARAM_MDSCMAX] = QP(593);

	// MDSC base recruitment
	_paramFloatInternal[PARAM_K_BASE_REC] = QP(596);

	// MDSC recruitment by CCL2
	_paramFloatInternal[PARAM_K_REC_BY_CCL2] = QP(578);

	// half maximal effective concentration of CCL2 on recruitment of MDSC into the tumor (mol/m^3)
	_paramFloatInternal[PARAM_EC50_CCL2_REC] = QP(591);	

	// half maximal inhibitory concentration of NO on inhibition of CD8+ T cell cytotoxic activity (ng/ml)
	_paramFloatInternal[PARAM_IC50_NO_CTL] = QP(590)* _paramFloat[PARAM_NO_MOLECULAR_WEIGHT] * 1e6;

	// half maximal inhibitory concentration of Arg I on inhibition of CD8+ T cell cytotoxic activity (ng/ml)
	_paramFloatInternal[PARAM_IC50_ARGI_CTL] = QP(589)* _paramFloat[PARAM_ARGI_MOLECULAR_WEIGHT] * 1e6;	

	// half maximal effective concentration of arginase I on Treg expansion (ng/ml)
	_paramFloatInternal[PARAM_EC50_ARGI_TREG] = QP(592) * _paramFloat[PARAM_ARGI_MOLECULAR_WEIGHT] * 1e6;	
	
	// half maximal inhibitory concentration of entinostat on anti-proliferation of tumor cell (mol/m^3)
	_paramFloatInternal[PARAM_IC50_ENT_C] = QP(580);

		// half maximal inhibitory concentration of entinostat on inhibition of CCL2 production (mol/m^3)
	_paramFloatInternal[PARAM_IC50_ENT_CCL2] = QP(595);		

	// half maximal inhibitory concentration of entinostat on inhibition of NO production (mol/m^3)
	_paramFloatInternal[PARAM_IC50_ENT_NO] = QP(587);	

	// half maximal inhibitory concentration of entinostat on inhibition of Arg I production (mol/m^3)
	_paramFloatInternal[PARAM_IC50_ENT_ARGI] = QP(597);	

	// number of PD1/PDL1 binding for half maximal inhibition 
	_paramFloatInternal[PARAM_PD1_PDL1_HALF] = QP(558);

	// total number of PD1 per synapse
	_paramFloatInternal[PARAM_PD1_SYN] = QP(562)*QP(263)/QP(264);

	// total number of PDL1 per synapse
	_paramFloatInternal[PARAM_PDL1_SYN_MAX] = QP(566)*QP(263)/QP(265);
	
	// k1 for PDL1-PD1 calculation
	_paramFloatInternal[PARAM_PDL1_K1] = 1 / (QP(545)/QP(508)) / QP(263);

	// k2 for PDL1-PD1 calculation
	_paramFloatInternal[PARAM_PDL1_K2] = 1 / (QP(547)/QP(537)) / (QP(516)/2);

	// k3 for PDL1-PD1 calculation
	_paramFloatInternal[PARAM_PDL1_K3] = QP(555) * QP(537) / (2 * QP(547));
	// hill coefficient
	_paramFloatInternal[PARAM_N_PD1_PDL1] = QP(559);
	...
	*/
	...
	_paramFloatInternal[PARAM_TEFF_RECRUIT_K] = QP(46) * t_step_sec * site_per_port / w / _paramFloat[PARAM_ADH_SITE_DENSITY]; 
	// Treg -> k (1/mol) // p = k (1/mol) * Cent.T (mol)
	_paramFloatInternal[PARAM_TREG_RECRUIT_K] = QP(27) * t_step_sec * site_per_port / w / _paramFloat[PARAM_ADH_SITE_DENSITY];
	...
	// MDSC -> k (m^3/mol) // p = k (m^3/mol) * Tum.Vol (m^3) * (Tum.MDSCmax * Tum.Vol - Tum.MDSC) (mol)
	_paramFloatInternal[PARAM_MDSC_RECRUIT_K] = QP(596) * t_step_sec * site_per_port / w / _paramFloat[PARAM_ADH_SITE_DENSITY];
	_paramFloatInternal[PARAM_MDSC_RECRUIT_BY_CCL2_K] = QP(578) * t_step_sec * site_per_port / w / _paramFloat[PARAM_ADH_SITE_DENSITY];

	// maximum cancer number
	_paramFloatInternal[PARAM_TUM_MAX_C] = QP(15) * AVOGADROS;
	std::cout << "Max Cancer cell: " << _paramFloat[PARAM_TUM_MAX_C] << std::endl;
	...
	_paramFloatInternal[PARAM_MDSC_LIFE_MEAN] = 1 / QP(579) / t_step_sec;
